...
try:
    ...
    Window.fullscreen = False  # فعال نگه داشتن دکمه‌های ناوبری پایین صفحه
except Exception:
    pass
...
try:
    ...
    yekan_path = resources.resource_find('yekan.ttf') or resources.resource_find('fonts/yekan.ttf')
    if yekan_path:
        LabelBase.register(name='Roboto', fn_regular=yekan_path, fn_bold=yekan_path)
    else:
...
class MainMenuScreen(BoxLayout):
    ...
    def rebuild(self):
        ...
        btn_1_text = self.app.translate("4 Prop Scan")
        self.btn_1 = PlasticButton(text=btn_1_text, btn_color=[0.56, 0.85, 0.56, 1])
        self.btn_1.bind(on_release=lambda x: self.app.select_mode("4-prop"))
        
        btn_2_text = self.app.translate("2 prop scanning, point-to-point")
        self.btn_2 = PlasticButton(text=btn_2_text, btn_color=[0.11, 0.51, 0.84, 1])
        self.btn_2.bind(on_release=lambda x: self.app.show_popup(
            "Update" if self.app.lang == 'en' else "آپدیت",
            "This feature will be available in the next update." if self.app.lang == 'en' else "این قسمت در آپدیت بعدی ارائه میشود"
        ))
        
        btn_3_text = self.app.translate("Products")
        self.btn_3 = PlasticButton(text=btn_3_text, btn_color=[0.18, 0.65, 0.28, 1])
        self.btn_3.bind(on_release=self.app.show_products_popup)
        
        btn_4_text = self.app.translate("Contact Us")
        self.btn_4 = PlasticButton(text=btn_4_text, btn_color=[0.9, 0.7, 0.1, 1])
        self.btn_4.bind(on_release=self.app.show_contact_popup)
        
        btn_exit_text = self.app.translate("Exit")
        self.btn_exit = PlasticButton(text=btn_exit_text, btn_color=[0.84, 0.17, 0.12, 1], size_hint_y=None, height=dp(35))
...
class MainApp(App):
    ...
    def translate(self, text):
        ...
        translations = {
            ...
            "Products": "محصولات",
            "Recall Picture": "فراخوانی عکس",
            "Share": "اشتراک‌گذاری",
            "Delete": "حذف",
            "Load": "فراخوانی",
            "4 Prop Scan": "اسکن ۴ پراپ",
            "2 prop scanning, point-to-point": "اسکن دو پراپ، نقطه به نقطه",
            "Exit": "خروج از برنامه",
            "Update": "آپدیت",
            "This feature will be available in the next update.": "این قسمت در آپدیت بعدی ارائه میشود",
            "Prev": "قبلی",
            "Next": "بعدی",
            "Exit Delete": "خروج از حذف",
            "Delete Mode": "حالت حذف"
        }
                ...
                for line in lines:
                    ...
                    if arabic_reshaper is not None and get_display is not None:
                        try:
                            ...
                            reshaped_lines.append(line[::-1])
                    else:
                        reshaped_lines.append(line[::-1])
                reshaped_parts.append('\n'.join(reshaped_lines))
    ...
    def update_ui_language(self):
        ...
        self.btn_save_jpg.text = self.translate("Save Picture")
        self.btn_recall_jpg.text = self.translate("Recall Picture")
        
        if hasattr(self, 'btn_menu') and self.btn_menu:
            self.btn_menu.text = "Menu" if self.lang == 'en' else self.shape_text("منو")
    ...
    def setup_ui(self):
        ...
        lang = BoxLayout(spacing=dp(2), size_hint_x=0.15)
        ...
        self.btn_menu = PlasticButton(text="Menu", btn_color=[0.5, 0.5, 0.5, 1], size_hint_x=0.15)
        self.btn_menu.bind(on_release=lambda x: self.show_menu_screen())
        r5.add_widget(self.btn_menu)
        
        self.btn_reset = PlasticButton(text="Reset", btn_color=[0.9, 0.7, 0.1, 1], size_hint_x=0.275)
        self.btn_reset.bind(on_release=self.reset_all)
        self.btn_save = PlasticButton(text="Save Memory", btn_color=[0.1, 0.6, 0.2, 1], size_hint_x=0.275)
        self.btn_save.bind(on_release=self.save_memory_click)
        self.btn_recall = PlasticButton(text="Recall Memory", btn_color=[1, 0.4, 0.7, 1], size_hint_x=0.275)
        ...
        self.btn_save_jpg = PlasticButton(text="Save Picture", btn_color=[0.1, 0.6, 0.2, 1])
        self.btn_save_jpg.bind(on_release=self.save_jpeg_click)
        self.btn_recall_jpg = PlasticButton(text="Recall Picture", btn_color=[0.5, 0.0, 0.5, 1])
        self.btn_recall_jpg.bind(on_release=self.recall_jpeg_click)
        
        r6.add_widget(self.btn_undo)
        r6.add_widget(self.btn_redo)
        r6.add_widget(self.btn_save_jpg)
        r6.add_widget(self.btn_recall_jpg)
    ...
    def show_contact_popup(self, *args):
        ...
        tel_text = self.shape_text("شماره همراه : 09944194262") if self.lang == 'fa' else "Mobile : 09944194262"
        btn_tel2 = PlasticButton(text=tel_text, btn_color=[0.18, 0.65, 0.28, 1], size_hint_y=None, height=dp(40))
        btn_tel2.bind(on_release=lambda x: webbrowser.open("tel:09944194262"))
        list_box.add_widget(btn_tel2)
        
        tg_text = self.shape_text("کانال تلگرام") if self.lang == 'fa' else "Telegram Channel"
        btn_tg = PlasticButton(text=tg_text, btn_color=[0.11, 0.51, 0.84, 1], size_hint_y=None, height=dp(40))
        btn_tg.bind(on_release=lambda x: webbrowser.open("https://t.me/kanalCornix"))
        list_box.add_widget(btn_tg)
        
        rb_text = self.shape_text("کانال روبیکا") if self.lang == 'fa' else "Rubika Channel"
        btn_rb = PlasticButton(text=rb_text, btn_color=[0.5, 0.0, 0.5, 1], size_hint_y=None, height=dp(40))
        btn_rb.bind(on_release=lambda x: webbrowser.open("http://rubika.ir/cornix_2026"))
        list_box.add_widget(btn_rb)
        
        ig_text = self.shape_text("صفحه اینستاگرام") if self.lang == 'fa' else "Instagram"
        btn_ig = PlasticButton(text=ig_text, btn_color=[0.84, 0.17, 0.12, 1], size_hint_y=None, height=dp(40))
        btn_ig.bind(on_release=lambda x: webbrowser.open("https://instagram.com/cornixsistem"))
        list_box.add_widget(btn_ig)
        
        sv.add_widget(list_box)
        content.add_widget(sv)
        
        close_text = self.shape_text("بستن") if self.lang == 'fa' else "Close"
        btn_close = PlasticButton(text=close_text, btn_color=[0.8, 0.2, 0.2, 1], size_hint_y=0.15)
        
        popup = Popup(
            title=self.translate("Contact Us"),
            content=content,
            size_hint=(0.85, 0.75),
            auto_dismiss=True
        )
    ...
    def show_products_popup(self, *args):
        products_data = [
            {
                "title_en": "1. Winner PRO",
                "desc_en": "Smart and powerful scanner for detecting metals, water, and voids at high depths, featuring AI target size identification.",
                "title_fa": "۱. Winner PRO",
                "desc_fa": "اسکنر هوشمند و قدرتمند، برای آشکار سازی فلزات و آب و حفره، در اعماق بالا، برخورداری از هوش مصنوعی شناسایی سایز هدف",
                "img_name": "winner_pro"
            },
            {
                "title_en": "2. Zeuse",
                "desc_en": "Powerful long-range locator, reverse-engineered from the powerful Navigator system.",
                "title_fa": "۲. Zeuse",
                "desc_fa": "ردیاب قدرتمند و شعاع زنی بالا، مهندسی معکوس از ردیاب قدرتمند Navigator",
                "img_name": "zeuse"
            },
            {
                "title_en": "3. Winner",
                "desc_en": "Smart scanner for detecting metals, water, and voids.",
                "title_fa": "۳. Winner",
                "desc_fa": "اسکنر هوشمند برای آشکار سازی فلزات و آب و حفره",
                "img_name": "winner"
            },
            {
                "title_en": "4. Trojan",
                "desc_en": "Portable and powerful locator with Low and Medium frequencies.",
                "title_fa": "۴. Trojan",
                "desc_fa": "ردیاب پرتابل و قدرتمند، دارای فرکانس‌های Low و Medium",
                "img_name": "trojan"
            }
        ]
        ...
        close_text = self.shape_text("خارج شدن") if self.lang == 'fa' else "Close"
        next_text = self.shape_text("بعدی") if self.lang == 'fa' else "Next"
        prev_text = self.shape_text("قبلی") if self.lang == 'fa' else "Prev"
        
        btn_prev = PlasticButton(text=prev_text, btn_color=[0.11, 0.51, 0.84, 1])
        btn_close = PlasticButton(text=close_text, btn_color=[0.8, 0.2, 0.2, 1])
        btn_next = PlasticButton(text=next_text, btn_color=[0.18, 0.65, 0.28, 1])
        ...
        def update_view():
            ...
            if self.lang == 'fa':
                title_text = self.shape_text(f"[b][color=#FFD700]{prod['title_fa']}[/color][/b]")
                desc_text = self.shape_text(prod["desc_fa"])
                full_text = f"{title_text}\n{desc_text}"
            else:
                full_text = f"[b][color=#FFD700]{prod['title_en']}[/color][/b]\n{prod['desc_en']}"
            
            lbl_desc.text = full_text
            lbl_desc.halign = 'right' if self.lang == 'fa' else 'left'
    ...
    def set_lang(self, l):
        ...
        try:
            ...
            if l == 'fa':
                ...
                roboto_reg = resources.resource_find('fonts/Roboto-Regular.ttf') or \
                             resources.resource_find('data/fonts/Roboto-Regular.ttf')
                roboto_bold = resources.resource_find('fonts/Roboto-Bold.ttf') or \
                              resources.resource_find('data/fonts/Roboto-Bold.ttf')
                if roboto_reg and roboto_bold:
                    LabelBase.register(name='Roboto', fn_regular=roboto_reg, fn_bold=roboto_bold)
        except Exception as e:
            pass
    ...
    def refine_target_positions(self, dt):
        ...
        p1p2 = self.safe_float(self.ui_entry_values.get("p1p2", ""))
        p2p4 = self.safe_float(self.ui_entry_values.get("p2p4", ""))
        p4p3 = self.safe_float(self.ui_entry_values.get("p4p3", ""))
        p3p1 = self.safe_float(self.ui_entry_values.get("p3p1", ""))
        p1p4 = self.safe_float(self.ui_entry_values.get("p1p4", ""))
        p2p3 = self.safe_float(self.ui_entry_values.get("p2p3", ""))
        ...
        if (all(s is not None and 0 <= s <= 100000 for s in sides) and 
            all(d is not None and 0 <= d <= 100000 for d in diags)):
            min_side = min(sides)
            if p1p4 < min_side and p2p3 < min_side:
                is_special_case = True
                if abs(p1p4 - p2p3) < 1e-5:
                    special_cx = 0.5
                    special_cy = 0.5
                else:
                    if p1p4 < p2p3:
                    ...
                    side_dict = {"p1p2": p1p2, "p2p4": p2p4, "p4p3": p4p3, "p3p1": p3p1}
                    valid_sides = {k: v for k, v in side_dict.items() if v is not None}
                    if valid_sides:
                        max_side_key = max(valid_sides, key=valid_sides.get)
                        rep_dx, rep_dy = 0.0, 0.0
                        if max_side_key == "p3p1":
                            rep_dx = 1.0
                        elif max_side_key == "p2p4":
                            rep_dx = -1.0
                        elif max_side_key == "p1p2":
                            rep_dy = 1.0
                        elif max_side_key == "p4p3":
                            rep_dy = -1.0

                        dot = rep_dx * ux + rep_dy * uy
                        shift_mag = 0.38 * (1.0 - ratio)

                        if dot >= 0:
                            special_cx = 0.5 + shift_mag * ux
                            special_cy = 0.5 + shift_mag * uy
                        else:
                            special_cx = 0.5 - shift_mag * ux
                            special_cy = 0.5 - shift_mag * uy
        ...
        for circ in self.detected_circles:
            ...
            if is_special_case:
                ...
                radius_ratio = (diameter_m / 2.0) / max(min(self.width_val, self.length_val), 1e-9)
                circ["center_x_ratio"], circ["center_y_ratio"] = self.repel_circle_from_edges(special_cx, special_cy, radius_ratio)
                continue
    ...
    def calculate_geophysics_heatmap(self):
        ...
        for k in self.edge_keys:
            if vals.get(k) is None:
                all_6_valid = False
                break
        if all_6_valid and self.active_mode == "4-prop":
    ...
    def draw_river_pebble(self, draw, cx, cy, r_avg, index, fill_rgb, outline_rgb, alpha):
        ...
        for i in range(num_pts):
            angle = 2 * math.pi * i / num_pts
            noise_factor = 0.85 + 0.15 * math.sin(index * 1.9 + i * 1.3)
            size_variation = 0.9 + 0.2 * math.cos(index * 2.7)
            r = r_avg * noise_factor * size_variation
            rx = cx + r * math.cos(angle)
            ry = cy + r * math.sin(angle)
            vertices.append((rx, ry))
        
        draw.polygon(vertices, fill=outline_rgb + (alpha,))
        ...
        draw.polygon(inner_vertices, fill=fill_rgb + (alpha,))
        
        if index % 3 == 0:
            crack_pts = [
                (cx - r_avg * 0.4, cy - r_avg * 0.2),
                (cx, cy),
                (cx + r_avg * 0.2, cy + r_avg * 0.3)
            ]
            draw.line(crack_pts, fill=outline_rgb + (alpha,), width=1)
        ...
        if self.show_geo_var and self.heatmap_data is not None and not self.gpr_active and not self.fibo_active and not self.compare_active:
            ...
            for idx, (cx, cy) in enumerate(overlapping_zones):
                ...
                if self.lang == 'fa':
                    ...
                    raw_text = f"هدف {idx_str}:  [b][size=13sp]{translated_label}[/size][/b]      مقدار: {val_str}\nX = {x_m_str} متر        Y = {y_m_str} متر\nعمق (H) = {depth_str}"
                    target_text = self.shape_text(raw_text)
                else:
                    target_text = f"Target {idx+1}:  [b][size=13sp]{translated_label}[/size][/b]      Value: {val:.0f}\nX = {x_m:.2f} m        Y = {y_m:.2f} m\nDepth (H) = {depth_text}"
                cx_pix = x_avg * w
                cy_pix = h - y_avg * h
    ...
    def draw_pil_gpr(self, draw, w, h, img):
        ...
        if self.transition_step > 0:
            ...
            for j in range(render_ny):
                try:
                    ry_local = j / (render_ny - 1)
                except ZeroDivisionError:
                    ry_local = 0.5
                for i in range(render_nx):
                    try:
                        rx_local = i / (render_nx - 1)
                    except ZeroDivisionError:
                        rx_local = 0.5
    ...
    def get_pil_font(self, text_size, bold=False):
        try:
            ...
            font_path = resources.resource_find('yekan.ttf') or resources.resource_find('fonts/yekan.ttf') or resources.resource_find('fonts/DejaVuSans.ttf')
            if font_path:
                return ImageFont.truetype(font_path, text_size)
        except Exception:
            pass
        for font_name in ["yekan.ttf", "arialb.ttf" if bold else "arial.ttf", "LiberationSans-Bold.ttf" if bold else "LiberationSans-Regular.ttf", "sans-serif"]:
            try:
    ...
    def draw_rotated_text(self, img, text, position, angle, font, fill):
        ...
        try:
            if hasattr(temp_img_draw, 'textbbox'):
                box = temp_img_draw.textbbox((0, 0), text, font=font)
                tw = box[2] - box[0]
                th = box[3] - box[1]
            else:
                tw, th = temp_img_draw.multiline_textsize(text, font=font)
        except Exception:
            lines = text.split('\n')
    ...
    def build_report_layout(self):
        ...
        if self.lang == 'fa':
            ...
            txt_range = self.convert_digits(txt_range)
    ...
    def show_popup(self, title, text):
        ...
        lbl = Label(text=translated_text, halign='right' if self.lang == 'fa' else 'center', valign='middle', size_hint_y=0.7)
    ...
    def load_state(self, state):
        ...
        self.scan_grid = [row[:] for row in state["scan_grid"]] if state.get("scan_grid") is not None else None
        self.sampled_points = state.get("sampled_points", []).copy()
        self.final_min_points = state.get("final_min_points", []).copy()
        self.transition_step = state.get("transition_step", 0)
        self.gpr_fibo_skeleton_visible = state.get("gpr_fibo_skeleton_visible", False)
    ...
    def execute_save_memory(self, is_internal, filename):
        ...
        try:
            root_dir = self.get_storage_root()
            folder = os.path.join(root_dir, "Cornix Winner", "File Scan")
            ...
            try:
                fallback_folder = os.path.join(self.user_data_dir, "Cornix Winner", "File Scan")
    ...
    def recall_memory_with_location(self, is_internal):
        ...
        try:
            fallback_dir = os.path.join(self.user_data_dir, "Cornix Winner", "File Scan")
            if os.path.exists(fallback_dir):
                for f in os.listdir(fallback_dir):
                    if f.endswith(".json"):
                        files_map[f] = os.path.join(fallback_dir, f)
        except Exception:
            pass
            
        try:
            root_dir = self.get_storage_root()
            target_dir = os.path.join(root_dir, "Cornix Winner", "File Scan")
        ...
        self.recall_files_list = list(files_map.keys())
        self.recall_files_map = files_map
        ...
        self.btn_delete_mode = PlasticButton(text="Delete Mode" if self.lang=='en' else "حالت حذف", btn_color=[0.8, 0.2, 0.2, 1])
        ...
        def populate_file_list(query=""):
            ...
            for f in filtered_files:
                display_name = f
                btn_clr = [0.3, 0.3, 0.3, 1]
                if self.delete_mode:
                    display_name = f"🗑 {f}"
                    btn_clr = [0.6, 0.1, 0.1, 1]
                btn = PlasticButton(text=display_name, btn_color=btn_clr, size_hint_y=None, height=dp(40), markup=True)
                btn.bind(on_release=lambda instance, filename=f: on_file_click(filename))
        ...
        def on_file_click(filename):
            if self.delete_mode:
                ...
                load_and_dismiss(filename)

        def toggle_delete_mode(*args):
            self.delete_mode = not self.delete_mode
            if self.delete_mode:
                self.btn_delete_mode.btn_color = [0.1, 0.6, 0.2, 1]
                self.btn_delete_mode.text = "Exit Delete" if self.lang=='en' else "خروج از حذف"
            else:
                self.btn_delete_mode.btn_color = [0.8, 0.2, 0.2, 1]
                self.btn_delete_mode.text = "Delete Mode" if self.lang=='en' else "حالت حذف"
            populate_file_list(self.current_search_query)
        ...
        def trigger_search_keyboard(*args):
            def on_search_query_submitted(query):
                ...
                self.btn_search_trigger.text = self.translate("Search: [Click to type]") if not query else query
                populate_file_list(query)
                
            VirtualKeyboardPopup(title=self.translate("Type query / متن جستجو را وارد کنید"), callback=on_search_query_submitted, default_text=self.current_search_query).open()
    ...
    def show_scan_options_popup(self, filename):
        ...
        btn_load = PlasticButton(text="Load" if self.lang=='en' else "فراخوانی", btn_color=[0.18, 0.65, 0.28, 1])
        btn_delete = PlasticButton(text="Delete" if self.lang=='en' else "حذف", btn_color=[0.8, 0.2, 0.2, 1])
        btn_share = PlasticButton(text="Share" if self.lang=='en' else "اشتراک‌گذاری", btn_color=[0.5, 0.0, 0.5, 1])
        btn_cancel = PlasticButton(text="Cancel" if self.lang=='en' else "انصراف", btn_color=[0.5, 0.5, 0.5, 1])
        ...
        popup = Popup(title=self.translate("Select Scan File"), content=content, size_hint=(0.85, 0.45), auto_dismiss=False)
        
        def on_ok(*args):
            self.recall_select_popup.dismiss()
            self.load_scan_file(filename)
            popup.dismiss()
        ...
        def on_delete(*args):
            ...
            if filepath and os.path.exists(filepath):
                try:
                    ...
                    pass
            self.recall_select_popup.dismiss()
            self.recall_memory_with_location(True)
            popup.dismiss()
            
        btn_load.bind(on_release=on_ok)
        btn_delete.bind(on_release=on_delete)
        btn_cancel.bind(on_release=popup.dismiss)
        
        def on_share(*args):
            self.share_file_native(self.recall_files_map[filename], "application/json")
    ...
    def execute_save_jpeg(self, is_internal, filename):
        ...
        try:
            ...
            folder = os.path.join(root, "Cornix Winner", "picture scan")
            os.makedirs(folder, exist_ok=True)
            filepath = os.path.join(folder, filename)
            ...
            try:
                fallback_folder = os.path.join(self.user_data_dir, "Cornix Winner", "picture scan")
                os.makedirs(fallback_folder, exist_ok=True)
                filepath = os.path.join(fallback_folder, filename)
                self.monitor.export_to_png(filepath)
                title = "Success (Fallback)"
    ...
    def recall_jpeg_click(self, *args):
        content = BoxLayout(orientation='vertical', padding=10, spacing=10)
        lbl = Label(text=self.translate("Recall Picture"), size_hint_y=0.3, font_size='14sp', bold=True)
        content.add_widget(lbl)
        ...
        popup_loc = Popup(title=self.translate("Recall Picture"), content=content, size_hint=(0.85, 0.4))
    ...
    def recall_jpeg_with_location(self, is_internal):
        ...
        try:
            fallback_dir = os.path.join(self.user_data_dir, "Cornix Winner", "picture scan")
            if os.path.exists(fallback_dir):
                for f in os.listdir(fallback_dir):
        ...
        try:
            root_dir = self.get_storage_root()
            target_dir = os.path.join(root_dir, "Cornix Winner", "picture scan")
            if os.path.exists(target_dir):
                for f in os.listdir(target_dir):
                    if f.lower().endswith(('.png', '.jpg', '.jpeg')):
                        files_map[f] = os.path.join(target_dir, f)
        ...
        bottom_row = BoxLayout(spacing=dp(3), size_hint_y=0.15)
        self.btn_pic_delet_toggle = PlasticButton(text="Delete Mode" if self.lang=='en' else "حالت حذف", btn_color=[0.8, 0.2, 0.2, 1])
        btn_cancel_select = PlasticButton(text=self.translate("Cancel"), btn_color=[0.5, 0.5, 0.5, 1])
        ...
        def populate_pic_file_list(query=""):
            ...
            filtered_files = []
            for f in self.recall_pic_files_list:
                if not query or query.lower() in f.lower():
                    filtered_files.append(f)
                
            if not filtered_files:
                lbl_empty = Label(text=self.translate("No matches found"), size_hint_y=None, height=dp(40))
                self.pic_file_list_grid.add_widget(lbl_empty)
                return
        ...
        def toggle_pic_delete_mode(*args):
            ...
            if self.pic_delete_mode:
                self.btn_pic_delet_toggle.btn_color = [0.1, 0.6, 0.2, 1]
                self.btn_pic_delet_toggle.text = "Exit Delete" if self.lang=='en' else "خروج از حذف"
            else:
                self.btn_pic_delet_toggle.btn_color = [0.8, 0.2, 0.2, 1]
                self.btn_pic_delet_toggle.text = "Delete Mode" if self.lang=='en' else "حالت حذف"
            populate_pic_file_list(self.current_pic_search_query)
        ...
        def trigger_pic_search_keyboard(*args):
            ...
            VirtualKeyboardPopup(title=self.translate("Type query / متن جستجو را وارد کنید"), callback=on_pic_search_query_submitted, default_text=self.current_pic_search_query).open()

        def clear_pic_search(*args):
            self.current_pic_search_query = ""
            self.btn_pic_search_trigger.text = self.translate("Search: [Click to type]")
            populate_pic_file_list("")

        self.btn_pic_search_trigger.bind(on_release=trigger_pic_search_keyboard)
        btn_clear_pic_search.bind(on_release=clear_pic_search)
        ...
        def on_delete(*args):
            try:
                if os.path.exists(filepath):
                    os.remove(filepath)
                viewer_popup.dismiss()
                self.show_popup("Success", "File deleted successfully!" if self.lang=='en' else "فایل با موفقیت حذف شد!")
                self.recall_jpeg_with_location(is_internal=True)
            except Exception as e:
                self.show_popup("Error", str(e))
    ...
    def share_file_native(self, filepath, mime_type="application/json"):
        if py_platform.system() == 'Linux' and 'ANDROID_ARGUMENT' in os.environ:
            try:
                ...
                share_intent.setType(String(mime_type))
                
                file_obj = File(filepath)
                fileUri = Uri.fromFile(file_obj)
                
                Parcelable = autoclass('android.os.Parcelable')
                share_intent.putExtra(Intent.EXTRA_STREAM, cast(Parcelable, fileUri))
                share_intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
                
                chooser = Intent.createChooser(share_intent, String("Share File via" if self.lang=='en' else "اشتراک‌گذاری فایل از طریق"))
                currentActivity.startActivity(chooser)
            except Exception as e:
                self.show_popup("Error Sharing", f"Android share failed: {str(e)}" if self.lang=='en' else f"خطا در اشتراک‌گذاری: {str(e)}")
        else:
            try:
                import subprocess
                if is_windows:
